...
const validReport: ImpactReport = {
  ...
  keyChanges: [
    'Expired discount codes are now rejected, so they can no longer reduce the order total.',
  ],
  qaChecks: [
    'Verify an expired discount is rejected and the total stays unchanged.',
  ],
  uncertainty: [],
};

const input = {
  options: { apiKey: 'offline-test-key', logLevel: 'off' as const },
...
test('returns the first valid report, trims fields, and preserves formatting and tracing', async t => {
  const { requests, logs } = mockCompletions(t, [
    JSON.stringify({
      ...validReport,
  ...
  assert.ok(
    String(requests[0].body.messages[1].content).endsWith(
...
test('corrects one semantic failure using the same evidence and the assistant response', async t => {
  const badReport = {
    ...validReport,
    summary: 'QA Report: Checkout',
    qaChecks: ['Expired discounts'],
  };
...
test('throws after two semantic failures without a third request', async t => {
  ...
  await assert.rejects(
    generateImpactReport(input),
    /semantically invalid.*after 2 attempts: keyChanges\[0\] must not be empty/,
  );
  assert.equal(requests.length, 2);
...
const structuralFailures: [string, unknown][] = [
  [
    ...
    {
      summary: validReport.summary,
      keyChanges: validReport.keyChanges,
      qaChecks: validReport.qaChecks,
    },
  ],
  ...
  [
    ...
    { ...validReport, keyChanges: Array(6).fill(validReport.keyChanges[0]) },
  ],
  ['empty QA array', { ...validReport, qaChecks: [] }],
  [
    'too many QA checks',
    { ...validReport, qaChecks: Array(6).fill(validReport.qaChecks[0]) },
  ],
  [
    'too many uncertainties',
    { ...validReport, uncertainty: Array(2).fill('The evidence omits the rollout date.') },
  ],
];
...
for (const [name, content, error] of [
  ...
  ...structuralFailures.map(
    ([name, report]) => [name, JSON.stringify(report), { name: 'ZodError' }] as const,
  ),
] as const) {
...
test('does not retry malformed JSON returned on the correction attempt', async t => {
  const { requests, logs } = mockCompletions(t, [
    JSON.stringify({ ...validReport, summary: '' }),
    'not JSON',
  ]);
...
test('rejects empty values and heading or Markdown content in every field', () => {
  const invalidValues = [
    ...
    'QA Report',
    'QA Change: Checkout',
    'What changed: Checkout',
    'Change: Checkout',
    'Impact: Checkout',
    'Summary: Checkout',
    'qaChecks: Checkout',
    'QA checks: Checkout',
    'Uncertainty: Checkout',
    'Overview: Checkout',
    'Title: Checkout',
    '- Checkout',
    '1. Checkout',
    '> Checkout',
    'Verify **discounts** work.',
    'Verify *discounts* work.',
    'Verify _discounts_ work.',
    'Verify __discounts__ work.',
    'Verify `discounts` work.',
    'Verify ~~discounts~~ work.',
    'Verify [discounts](https://example.com) work.',
    'Verify <b>discounts</b> work.',
    'Verify discounts.\nImpact: Checkout',
  ];
  for (const value of invalidValues) {
    for (const field of ['summary', 'keyChanges', 'qaChecks', 'uncertainty'] as const) {
      const report = {
        ...validReport,
        [field]: field === 'summary' ? value : [value],
      };
      assert.ok(
        validateImpactReportSemantics(report).some(error => error.startsWith(field)),
...
test('accepts action verbs case-insensitively but requires a word boundary', () => {
  for (const verb of ['Verify', 'check', 'CONFIRM', 'Validate', 'Test', 'Ensure']) {
    assert.deepEqual(
      validateImpactReportSemantics({
        ...validReport,
        qaChecks: [`  ${verb} expired discounts are rejected.  `],
      }),
      [],
    );
  }
  for (const value of [
    'Checking discounts works.',
    'Verified discounts.',
    'Tests pass.',
    'Checkout discounts.',
  ]) {
    assert.match(
      validateImpactReportSemantics({ ...validReport, qaChecks: [value] }).join(' '),
      /qaChecks\[0\] must start with/,
    );
  }
});
...
test('accepts decimals, abbreviations, precise quantities, and genuine uncertainty', () => {
  assert.deepEqual(
    validateImpactReportSemantics({
      ...
      keyChanges: [
        'Eligible customers now pay $3.50 less when the discount rate is 1.2.',
      ],
      qaChecks: ['Verify eligible orders use a discount rate of 1.5.'],
      uncertainty: [
        'The evidence does not show whether existing discounts are recalculated.',
      ],
    }),
    [],
  );
...
test('rejects an incomplete key change without imposing a character limit', () => {
  const keyChange =
    'Cancellation behavior is reversed: orders in SHIPPED status will now throw the cannot-cancel error while orders in every other non-cancelled status can be cancelled, potentially allowing cancellation of orders already shipped under the old,';
  ...
  const errors = validateImpactReportSemantics({
    ...
    qaChecks: [
      'Confirm whether test expectations in test.ts#runTests use the previous rate.',
    ],
  });
  assert.ok(
    errors.some(
      error =>
        error ===
        'keyChanges[0] must be a complete sentence ending with a period, question mark, or exclamation mark.',
    ),
  );
  assert.ok(errors.some(error => error.startsWith('qaChecks[0] must be plain text')));
  assert.ok(
    errors.some(error => error.startsWith('qaChecks[0] must verify runtime behavior')),
  );
});
...
test('rejects source-maintenance and automated-test recommendations', () => {
  for (const qaCheck of [
    ...
    assert.match(
      validateImpactReportSemantics({ ...validReport, qaChecks: [qaCheck] }).join(' '),
      /must verify runtime behavior/,
    );
  }
});
