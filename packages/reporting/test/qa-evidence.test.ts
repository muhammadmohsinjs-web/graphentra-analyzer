...
import type { ApplicationContext } from '../src/contracts';
import {
  buildLLMPayload,
  getSourceRole,
  qaInstruction,
} from '../src/qa-evidence';

const catalog: Entity = {
  id: 'src/service.ts#getCatalog',
  name: 'getCatalog',
  kind: 'function',
  file: 'src/service.ts',
  startLine: 1,
...
const server: Entity = {
  ...catalog,
  id: 'src/server.ts#createServer',
  name: 'createServer',
  file: 'src/server.ts',
};
const tests: Entity = {
  ...catalog,
  id: 'tests/test.ts#runTests',
  name: 'runTests',
  file: 'tests/test.ts',
};
const context: ApplicationContext = {
  ...
  application: {
    name: 'Demo',
    summary: 'Catalog and checkout',
    purpose: 'Catalog and checkout',
  },
  domains: [
    {
      id: 'catalog',
      name: 'Catalog',
      description: 'Catalog plus unrelated checkout description',
    },
    { id: 'checkout', name: 'Checkout', description: 'Checkout' },
  ],
  terminology: [
    { term: 'stock', meaning: 'Available units' },
    { term: 'cart', meaning: 'Cart entries' },
  ],
  entityAnnotations: [
    {
      entityId: catalog.id,
      businessMeaning: 'Classify stock',
      domainIds: ['catalog'],
      confidence: 'high',
    },
    {
      entityId: checkout.id,
      businessMeaning: 'Check cart entry count',
      domainIds: ['checkout'],
      confidence: 'high',
    },
  ],
...
test('multiple changed functions produce one payload with isolated change evidence', () => {
  const payload = buildLLMPayload(
    [impactFor(catalog), impactFor(checkout)],
    context,
  );
  assert.equal(payload.analysisScope.changedFunctionCount, 2);
  assert.deepEqual(
    payload.changes.map(item => item.change.changedLines),
    [[2], [5]],
  );
  ...
  assert.deepEqual(
    payload.applicationContext.entityAnnotations.map(item => item.entityId),
    [catalog.id, checkout.id],
  );
...
test('test callers remain in every graph path but are classified separately in QA evidence', () => {
  ...
  assert.deepEqual(
    change.productionDependents.map(entity => entity.id),
    [server.id],
  );
  assert.deepEqual(
    change.testDependents.map(entity => entity.id),
    [tests.id],
  );
  ...
  assert.equal(
    buildLLMPayload([{ ...impact, changedEntity: tests }], context).changes[0]
      .changedEntity.sourceRole,
    'test',
  );
});
...
test('instructions preserve evidence authority, precise quantities, and plain field content', () => {
  ...
  assert.match(
    qaInstruction,
    /Do not\s+produce a section, summary, impact statement, or QA checklist for every changed function/,
  );
