...
async function readArtifact(directory: string, filename: string) {
  ...
  const arrays = filename === 'technical-graph.json' ? ['entities', 'relations']
    : filename === 'analysis.json' ? ['changedFiles', 'changedEntities', 'impacts']
  ...
  } else if (!isObject(data.repository) || typeof data.repository.headSha !== 'string' || !data.repository.headSha) {
    throw new Error('Expected repository.headSha.');
  }
  return { data, mtimeMs: (await stat(file)).mtimeMs };
}

export function createVisualizerServer(options: { target?: string; visualizerDirectory?: string } = {}) {
  const directory = path.resolve(options.target ?? process.cwd(), '.graphentra');
  const visualizerDirectory = options.visualizerDirectory ?? path.join(__dirname, '../visualizer');
  ...
  return createServer(async (request, response) => {
    ...
    if (host !== `127.0.0.1:${port}` && host !== `localhost:${port}`) {
      json(403, { error: 'Unexpected Host. Use the localhost or 127.0.0.1 URL with the listening port.' });
      return;
    }
      ...
      for (const filename of ['analysis.json', 'application-context.json']) {
        try {
          ...
          if (analysisRepository.headSha !== graphRepository.headSha) {
            warnings.push('analysis.json headSha does not match the graph; analysis was ignored.');
            continue;
          }
          analysis = artifact.data;
          warnings.push('Matching headSha values do not prove working-tree alignment between the graph and analysis.');
          if (typeof graphRepository.generatedAt === 'string'
            && Date.parse(graphRepository.generatedAt) > artifact.mtimeMs) {
            warnings.push('The graph generatedAt is later than the analysis.json modification time; analysis is potentially stale.');
          }
        } catch (error) {
    ...
    try {
      ...
      json(404, { error: 'Visualizer asset not found. Ensure the visualizer directory is installed alongside src or dist.' });
    }
  });
}
  ...
  try {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (argument === '--port' || argument.startsWith('--port=')) {
        ...
        const value = argument === '--port' ? args[++i] : argument.slice('--port='.length);
    ...
    server.listen(port, '127.0.0.1', () => {
      console.log(`Visualizer: http://127.0.0.1:${port} (target: ${path.resolve(target ?? process.cwd())})`);
    });
  } catch (error) {
